...
The fifth migration adds encrypted outbound jobs and hashed portal password-reset tokens. Job payloads are opaque `bytea`; lease and retry columns support delivery without exposing recipient or message content. Reset rows are single-use and expire automatically.
